...
function addWheel(){   
    ...
    if ((wheels.length < 8)){
        (<HTMLInputElement>document.getElementById("wheelError")).innerHTML = "Please, complete all the fiels correctly";          
    }else{
        let lastPos: number = carArray.length - 1;
        let car: any = carArray[lastPos];
        car.addWheel(wheels);       
        
        (<HTMLInputElement>document.getElementById("messageWheel")).innerHTML = `<br>WHEELS
        <br><br><b>Wheel 1:</b>
        <br>Brand: ${wheels[0]} 
        ...
        (<HTMLInputElement>document.getElementById("sectionB1")).style.display = "block";     
        (<HTMLInputElement>document.getElementById("sectionB2")).style.display = "none";  
        cleanWheelForm();
    }
    // revise the result /////////////////////
